...
async def generate_youtube_summary(video_url: str, custom_summary: str = None):
...
async def generate_reddit_summary(subreddit_name: str, custom_summary: str = None):
    """Generate a summary for a subreddit"""
    try:
        result = api_utils.generate_reddit_summary(subreddit_name)
        if result["status"] == "error":
            raise HTTPException(status_code=400, detail=result["message"])
        return result
    except Exception as e:
